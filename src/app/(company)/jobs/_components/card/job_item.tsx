    ...
    return (
        <Card>
                    ...
                    <div className="space-y-2 w-full">
                        ...
                        <div className="flex gap-4">
                            ...
                            <CardDescription className="flex gap-1">
                                ...
                                {job.location.type === 'REMOTE' ? <span>{job.location.type}</span> :
            ...
            <CardContent className="space-y-4">
                <>
                    {job.employment_type.toLowerCase() === 'full-time' && <Badge variant={'full_time'}>{job.employment_type}</Badge>}
                    {job.employment_type.toLowerCase() === 'part-time' && <Badge variant={'part_time'}>{job.employment_type}</Badge>}
                    {job.employment_type.toLowerCase() === 'internship' && <Badge variant={'internship'}>{job.employment_type}</Badge>}
                    {job.employment_type.toLowerCase() === 'contractor' && <Badge variant={'contractor'}>{job.employment_type}</Badge>}
                </>
                ...
                <div className="flex flex-col gap-2">
                    <CardDescription className="flex justify-between">
                        ...
                        <span
                            className="font-bold text-primary">{job.salary.min} - {job.salary.max} {job.salary.frequency}</span>
                    </CardDescription>
                    <CardDescription className="flex justify-between">
                        <span>Posted:</span>
